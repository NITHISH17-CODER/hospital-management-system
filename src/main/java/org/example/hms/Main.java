...
public class Main {
    public static void main(String[] args) {
        ...
        app.get("/api/patients", ctx -> {
            ...
            try (Connection conn = DatabaseManager.getConnection();
                ...
                while (rs.next()) {
                    ...
                    patient.put("contact", rs.getString("contact"));
                    patient.put("registration_date", rs.getString("registration_date"));
        ...
        app.post("/api/patients", ctx -> {
            ...
            try (Connection conn = DatabaseManager.getConnection();
                 PreparedStatement pstmt = conn.prepareStatement("INSERT INTO patients (name, age, gender, contact, password) VALUES (?, ?, ?, ?, ?)")) {
        ...
        app.post("/api/auth/login", ctx -> {
            ...
            try (Connection conn = DatabaseManager.getConnection()) {
                if (role.equals("patient")) {
                    ...
                    try (PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM patients WHERE name = ? AND contact = ?")) {
                        ...
                        try (ResultSet rs = pstmt.executeQuery()) {
                            if (rs.next()) {
                                ...
                                ctx.status(401).json(Map.of("error", "Patient not found with this combination of Full Name and Mobile Number."));
                            }
                        }
                    }
                    ...
                    try (PreparedStatement pstmt = conn.prepareStatement("SELECT * FROM doctors WHERE id = ? AND email = ? AND password = ?")) {
                        ...
                        try (ResultSet rs = pstmt.executeQuery()) {
                            if (rs.next()) {
                                ...
                                ctx.status(401).json(Map.of("error", "Invalid Doctor constraints (ID, Email, or Password mismatch)"));
                            }
                        }
                    }
                } else {
        ...
        app.get("/api/appointments", ctx -> {
            ...
            String query = "SELECT a.id, p.name as patient_name, d.name as doctor_name, a.appointment_date, a.status, a.problem " +
                           "FROM appointments a " +
        ...
        app.post("/api/appointments", ctx -> {
             Map body = ctx.bodyAsClass(Map.class);
             try (Connection conn = DatabaseManager.getConnection();
                  PreparedStatement pstmt = conn.prepareStatement("INSERT INTO appointments (patient_id, doctor_id, appointment_date, problem) VALUES (?, ?, ?, ?)")) {
                 ...
                 try (Statement bs = conn.createStatement();
                     ...
                     if(brs.next()) {
                         ...
                         double randomAmount = 150.0 + (Math.round(Math.random() * 300.0));
                         try (PreparedStatement bp = conn.prepareStatement("INSERT INTO billing (patient_id, appointment_id, amount, status) VALUES (?, ?, ?, 'UNPAID')")) {
                             bp.setInt(1, Integer.parseInt(body.get("patient_id").toString()));
                             bp.setInt(2, aptId);
                             bp.setDouble(3, randomAmount);
                             bp.executeUpdate();
                         }
                     }
                 } catch(Exception e) {}
        ...
        app.put("/api/appointments/{id}/cancel", ctx -> {
            ...
            try (Connection conn = DatabaseManager.getConnection();
                 PreparedStatement pstmt = conn.prepareStatement("UPDATE appointments SET status = 'CANCELLED' WHERE id = ?")) {
                pstmt.setInt(1, Integer.parseInt(id));
                pstmt.executeUpdate();
                ctx.json(Map.of("message", "Appointment cancelled successfully"));
            } catch(Exception e) {
                ctx.status(500).json(Map.of("error", e.getMessage()));
            }
        });
        ...
        app.get("/api/billing", ctx -> {
            ...
            String query = "SELECT b.id, p.name as patient_name, d.name as doctor_name, a.appointment_date, b.amount, b.status " +
                           "FROM billing b " +
        ...
        app.put("/api/billing/{id}/pay", ctx -> {
            String id = ctx.pathParam("id");
            try (Connection conn = DatabaseManager.getConnection();
                 PreparedStatement pstmt = conn.prepareStatement("UPDATE billing SET status = 'PAID' WHERE id = ?")) {
                pstmt.setInt(1, Integer.parseInt(id));
                pstmt.executeUpdate();
                ctx.json(Map.of("message", "Bill paid completely"));
            } catch(Exception e) {
                ctx.status(500).json(Map.of("error", e.getMessage()));
            }
        });
    ...
    private static void seedDoctors() {
        try (Connection conn = DatabaseManager.getConnection();
            ...
            System.err.println("Note: Could not seed doctors. " + e.getMessage());
        }
    }
